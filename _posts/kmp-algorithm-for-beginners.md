...
讓完整字串的前綴去跟 `ab` 的後綴比，若相同，則 LPS 長加一；若不同，且正被比較的完整字串的**前綴索引為 `0`**，那 LPS 長便為 0。接著繼續拿完整字串的前綴去跟 `abc` 的後綴比：
...
（前面沒說明的第六個字符設為 0，其實也要經過同樣的過程，只是因為再次比較時仍失敗，而此時完整字串的前綴索引為 `0`，因此才會結果看起來都一樣）
