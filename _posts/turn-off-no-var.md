...
清楚多了吧！現在你可以很篤定 `today` 絕對不會被這個區塊以外的地方用到，能放心又快速地把這兩行程式碼提取成 `recordDueDate` 函數。
...
至少我自己是幾乎沒有碰過重新宣告的問題。即便你的函數都寫得很長，但當你很有意識地在區分變數的作用域範圍，要出錯的機率可說是微乎其微。如果你真的很擔心程式壞掉，那你可以將所有用 `var` 宣告的變數都寫在函數最上方，總不會在這短短幾行中也有同名的問題吧？
...
同樣地，重新賦值的問題也是如此。想想看，你會在什麼情況下，不小心將一個不該被重新賦值的變數重新賦值？大多數情況下，都是你想重新賦值，但卻發現這個變數被宣告成 `const`，於是你只好回過頭把變數宣告成 `let`，再重新賦值。而且，即便你真的不小心犯錯了，會很難察覺嗎？會很難追蹤嗎？會很難改回來嗎？
